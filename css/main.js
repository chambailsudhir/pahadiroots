...
var RAZORPAY_KEY        = '';    // loaded from store-data (env var) — never hardcoded
...
async function loadData() {
  ...
  try {
    ...
    // ── Load Razorpay key from env (served via store-data, never hardcoded) ──
    if (data.razorpay_key) RAZORPAY_KEY = data.razorpay_key;
    var updated  = false;
...
async function checkoutRazorpay() {
  ...
  var options = {
    ...
    notes: {
      ...
      name:    name.trim(),
      phone:   phone.trim(),
      items:   JSON.stringify(cart.map(function(i){ return {id:i.id||null,variantId:i.variantId||null,qty:i.qty,price:i.price,name:i.name||''}; })),
    },
    theme: { color: '#2d6a4f' },
...
async function checkout() {
  ...
  var shipCharge = (subtotal > 0 && threshold > 0 && subtotal < threshold) ? ((!isNaN(FLAT_SHIP_CHARGE)) ? FLAT_SHIP_CHARGE : 99) : 0;
  var final      = subtotal + shipCharge;
  var gstAmount  = calcGST(cart).total; // GST inclusive — for invoice (cart used here; cartSnapshot declared below)
  ...
  showToast('✅ Order placed! Saving...');

  // Save to DB — wait for orderId before redirecting to confirmation page
  saveOrderToDB(name, phone, email, addr, city, state, pin, final, discount, shipCharge, 'whatsapp_cod', null, cartSnapshot, gstAmount)
    ...
    .catch(function(e) {
      ...
      _orderProcessing = false;
      window.location.href = '/order-confirmation?total=' + final;
    });
}
...
function trackOrder() {
  ...
  // Use public_get_order action — no admin password needed, safe for frontend
  fetch('/api/admin-api', {
    method: 'POST',
    headers: {'Content-Type':'application/json'},
