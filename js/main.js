...
function initCollectionImages(settings) {
  ...
  if (!document.getElementById('ccat-style')) {
    ...
    st.id = 'ccat-style';
    st.textContent = [
      '#cgrid{display:flex;gap:16px;overflow-x:auto;padding:12px 0 28px;scrollbar-width:none;-webkit-overflow-scrolling:touch;}',
      '#cgrid::-webkit-scrollbar{display:none;}',
      '.cc-cell{flex:0 0 calc((100% - 72px) / 5.5);min-width:0;display:flex;flex-direction:column;align-items:center;gap:10px;transition:transform .2s ease;}',
